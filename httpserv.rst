...
.. code-block:: c
    ...
    char *other_string = /* ... */;
    int ret = strncmp(other_string, "GET", 3);
    if(ret == 0) {
        printf("other_string begins with \"GET\"\n");
    }
    int other_string_len = strlen(other_string);
    if(other_string_len > 14) {
            ret = strncmp(other_string + 10, "HTTP", 4);
            if(ret == 0) {
                printf("other_string has the characters \"HTTP\" ten characters after the start.\n");
            }
    }
    int value = 42;
    char buf[8];
